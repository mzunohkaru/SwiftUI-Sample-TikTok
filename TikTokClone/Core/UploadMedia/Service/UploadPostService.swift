...
struct UploadPostService {
    ...
    func uploadPost(caption: String, videoUrlString: String) async throws {
        ...
        do {
            ...
            guard let videoUrl = try await VideoUploader.uploadVideoToStorage(withUrl: url, postId: ref.documentID) else { return }
    ...
    func updateThumbnailUrl(fromVideoUrl videoUrl: String, postId: String) async throws {
        do {
            ...
            guard let thumbnailUrl = try await ImageUploader.uploadImage(image: image, type: .post(path: postId)) else { return }
